        ...
        <div class="col-xl-12">
            <div class="card mg-b-20">
                <div class="card-header pb-0 justify-content-between">
                    <h3 class=" btn btn-primary-gradient btn-block " style="text-align:center;text-align: center;display: flex;flex-direction: column;">اضافة الارباح</h3>
                </div>
                <div class="card-body">
                    <form method="post" action="{{route('profits.update',$profit->id)}}">
                        @csrf
                        @method('put')
                        <div class="form-group">
                            ...
                            <select class="form-control" name="month"  id="exampleFormControlSelect1">
                                @foreach ($months as $monthNumber => $monthName)
                                    <option value="{{ $monthNumber }}" @selected($monthNumber==$profit->month)>{{ $monthName   }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" name="year" id="exampleFormControlSelect2">
                                @foreach ($years as $year)
                                    <option value="{{ $year }}"  @selected($year == $profit->year)>{{ $year }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">المبلغ</label>
                            <input type="text" name="money" class="form-control" value="{{ $profit->money }}" id="exampleInputEmail1"  placeholder="يرجى ادخال القيمة">

                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1"> صافي الربح</label>
                            <input type="text" name="money_after_discount" class="form-control" value="{{ $profit->money_after_discount }}" readonly id="exampleInputEmail1"  placeholder="يرجى ادخال القيمة">
